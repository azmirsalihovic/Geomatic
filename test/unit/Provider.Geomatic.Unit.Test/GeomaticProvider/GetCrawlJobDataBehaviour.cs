...
{
    public class GetCrawlJobDataBehaviour : GeomaticProviderTest
    {
        ...
        [Theory]
        [InlineAutoData(GeomaticConstants.KeyName.ApiKey, nameof(GeomaticCrawlJobData.ApiKey))]
        ...
        [Theory]
        [InlineAutoData]
        public async Task GeomaticCrawlJobDataReturned(Dictionary<string, object> dictionary, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            Assert.IsType<CluedIn.Crawling.Geomatic.Core.GeomaticCrawlJobData>(
                await Sut.GetCrawlJobData(_context, dictionary, organizationId, userId, providerDefinitionId));
        }
    }
}
